...
struct Vis2dNode
{
  ...
  void image_cb(const sensor_msgs::ImageConstPtr& img_msg)
  {
    std_msgs::Bool b;
    b.data = true;
    if (!img_queue.empty() && img_msg->header.stamp < img_queue.front()->header.stamp)
      reset(b);
  ...
  void trajectory_cb(const mlr_msgs::Point2dArray& msg)
  {
    ...
    {
      tracks[msg.ids[i]].push_back( cv::Point2f(msg.x[i],msg.y[i]) );
    }
  }
  ...
  void object_cb(const mlr_msgs::ObjectIds& msg)
  {
    ...
    for(int i=0; i<int(msg.offsets.size())-1; ++i)
    {
      ...
      cv::Scalar cvc = cv::Scalar(c.b,c.g,c.r);
      for(int j=msg.offsets[i]; j<msg.offsets[i+1]; ++j)
      {
        int id = msg.ids[j] >> 32;
        if(tracks.count(id) > 0)
        {
          for(size_t i = 0; i<tracks[id].size()-1; ++i)
          {
            cv::line(img->image, tracks[id][i], tracks[id][i+1], cvc, 1, CV_AA);
          }
          cv::circle(img->image, tracks[id].back(), 5, cvc, 1, CV_AA);
        }
      }
    }
  ...
  std::unordered_map<int,std::vector<cv::Point2f> > tracks;
};


int main(int argc, char** argv)
{
  ros::init(argc, argv, "vis_2d_tracks");
  Vis2dNode n;
  ros::Rate r(30); // hz
  while (n.nh_.ok())
  {
